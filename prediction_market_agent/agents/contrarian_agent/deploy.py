...
class ContrarianAgent(DeployableTraderAgent):
    ...
    YES_OVERPRICED_THRESHOLD = 0.65
    CONTRARIAN_P_YES = 0.35
    CONFIDENCE = 0.60
    ...
    def answer_binary_market(self, market: AgentMarket):
        ...
        # Only act when YES looks overpriced enough
        if market_prob < self.YES_OVERPRICED_THRESHOLD:
            print("Skipping: YES not overpriced enough for a contrarian NO bet.")
            return None

        print("Betting NO")

        return ProbabilisticAnswer(
            p_yes=Probability(self.CONTRARIAN_P_YES),
            confidence=self.CONFIDENCE,
            reasoning=(
                f"Contrarian NO bet: market YES probability "
                f"({market_prob:.3f}) exceeds threshold "
                f"({self.YES_OVERPRICED_THRESHOLD:.2f})."
            ),
        )

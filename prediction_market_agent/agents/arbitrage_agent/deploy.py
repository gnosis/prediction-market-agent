...
class DeployableOmenArbitrageAgent(DeployableTraderAgent):
    ...
    def __init__(
        ...
        super().__init__(enable_langfuse=enable_langfuse, place_bet=place_bet)
    ...
    def get_correlated_markets(self, market: AgentMarket) -> list[CorrelatedMarketPair]:
        ...
        for related_market in omen_markets:
            ...
            if related_market.market_maker_contract_address_checksummed != market.market_maker_contract_address_checksummed and result.correlation >= self.correlation_threshold:
    ...
    def build_trades_for_correlated_markets(self, pair: CorrelatedMarketPair) -> list[Trade]:
        ...
        return [
            Trade(
                trade_type=TradeType.BUY, outcome=True, amount=TokenAmount(amount=amount_yes, currency=market_to_bet_yes.currency)),
            Trade(
                trade_type=TradeType.BUY, outcome=False, amount=TokenAmount(amount=amount_no, currency=market_to_bet_no.currency)),
        ]

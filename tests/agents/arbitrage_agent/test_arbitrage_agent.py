...
from prediction_market_agent.agents.arbitrage_agent.deploy import DeployableOmenArbitrageAgent
...
def arbitrage_agent() -> t.Generator[DeployableOmenArbitrageAgent, None, None]:
    with patch(
        ...
        new=lambda x: None
    ), patch("prediction_market_agent_tooling.tools.langfuse_.get_langfuse_langchain_config"):
...
@pytest.mark.parametrize("related_market_fixture_name, is_correlated",
                         [("related_market",  True),
                          ("unrelated_market", False)])
def test_correlation_for_similar_markets(arbitrage_agent: DeployableOmenArbitrageAgent,
    ...
    correlation = arbitrage_agent.calculate_correlation_between_markets(market=main_market, related_market=other_market)
    assert arbitrage_agent.is_correlation_above_threshold(correlation.correlation) == is_correlated

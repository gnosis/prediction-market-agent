...
def test_get_all(
    prompt_table_handler: PromptTableHandler, example_prompts: list[Prompt]
) -> None:
    p = example_prompts[0].model_copy()
    with prompt_table_handler.sql_handler.db_manager.get_session() as session:
        session.add(p)
        session.commit()
    assert len(prompt_table_handler.sql_handler.get_all()) == 1


def test_query_empty_database(prompt_table_handler: PromptTableHandler) -> None:
    # Assert: Verify the database is empty
    assert len(prompt_table_handler.sql_handler.get_all()) == 0
...
def test_get_first(
    ...
    last_prompt = get_first(prompt_table_handler, column_to_order, True)
    ...
    last_prompt = get_first(prompt_table_handler, column_to_order, False)
    assert last_prompt is not None

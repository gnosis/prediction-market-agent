...
A library for exploring the landscape of AI Agent frameworks, using the example application of a prediction market betting agent. The various agents interact with markets from [Presagio (with former name Omen)](https://presagio.pages.dev/), [Manifold](https://manifold.markets/), and [Polymarket](https://polymarket.com/).

These agents build on top of the prediction market APIs from <https://github.com/gnosis/prediction-market-agent-tooling>.

## Setup

Install the project dependencies with [uv](https://docs.astral.sh/uv/), using Python 3.11:
...
Create a `.env` file in the root of the repo with the following variables as a minimal requirement:
...
- [deprecated] An autonomous agent with function calling. Can be 'prodded' by the user to guide its strategy: `streamlit run prediction_market_agent/agents/microchain_agent/app.py` (Deployed [here](https://autonomous-trader-agent.ai.gnosisdev.com))

## Dune Dashboard

The on-chain activity of the deployed agents from this repo can be tracked on a Dune dashboard [here](https://dune.com/gnosischain_team/ai-agents-overview-omen-prediction-markets).
